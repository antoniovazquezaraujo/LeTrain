...
class TopologyServiceTest {
    ...
    void testForkTopology() {
        ...
        tNorth.addRoute(Dir.S, Dir.N);
        tNorth.setPosition(new Point(1, 0));
        tSouth.addRoute(Dir.N, Dir.S);
        tSouth.setPosition(new Point(1, 2));
        tEast.addRoute(Dir.W, Dir.E);
        tEast.setPosition(new Point(2, 1));
        ...
        // Conexiones físicas
        fork.connect(Dir.N, tNorth);
        tNorth.connect(Dir.S, fork);
        fork.connect(Dir.S, tSouth);
        tSouth.connect(Dir.N, fork);
        fork.connect(Dir.E, tEast);
        tEast.connect(Dir.W, fork);
