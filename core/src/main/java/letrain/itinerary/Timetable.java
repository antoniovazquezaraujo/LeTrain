 ...
 *
 * <p>
 * Waypoint times are <b>time-of-day</b> values ({@code H:MM} / {@code HH:MM}, 24 h). The schedule is
 * read <b>in sequence</b>: a time smaller than the previous one belongs to the <b>next day</b>
...
public final class Timetable {
     ...
     * (an absolute minute count where day 0 is {@code [0, 1440)}). Returns the first absolute minute
     * at or after the previous one whose time of day equals {@code time}: if the time of day is
     * smaller than the previous one, it belongs to the next day. Equal times stay on the same day
     * (zero dwell).
     */
    public static long resolveAfter(long previousAbsoluteMinute, LocalTime time) {
        long dayStart =
                Math.floorDiv(previousAbsoluteMinute, MINUTES_PER_DAY) * MINUTES_PER_DAY;
