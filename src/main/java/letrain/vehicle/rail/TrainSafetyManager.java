...
/**
 * Interfaz para el gestor de seguridad y control de cantones (bloques) del tren. Define el contrato
 * para la reserva exclusiva de segmentos de vía, paradas preventivas y frenado de emergencia.
 */
public interface TrainSafetyManager {
    Logger log = LoggerFactory.getLogger(letrain.vehicle.rail.impl.TrainSafetyManager.class);

    /**
     * Determina si el tren tiene autorización de seguridad para moverse. Un tren en modo manual
     * siempre tiene permiso, mientras que en modo automático depende de la reserva del bloque.
     *
    ...
    /**
     * Restablece el estado de seguridad de emergencia interno del gestor en caso de invasión de vía
     * o conflicto.
     */
    void onEmergencyStop();

    /**
     * Reclama y reserva todos los cantones físicamente ocupados por los vagones y locomotoras del
     * tren. Se utiliza típicamente en la inicialización o al cargar una partida.
     */
    void claimOccupiedSegments();

    /**
     * Intenta bloquear y reservar los cantones iniciales (actual y el siguiente) para iniciar la
     * marcha de forma segura.
     */
    ...
    /**
     * Evento reactivo que se dispara cuando el tren entra físicamente en un nuevo segmento. Asegura
     * la posesión del nuevo segmento y solicita la reserva del cantón posterior.
     *
    ...
    /**
     * Reevalúa la disponibilidad del siguiente bloque cuando se libera un cantón y reanuda la
     * marcha si es posible.
     */
    void onBlockReleased();

    /**
     * Evento reactivo que se dispara al cambiar el sentido de la marcha. Libera el segmento
     * objetivo anterior y calcula/reserva el nuevo segmento frontal.
     */
    void onReverse();
    ...
    /**
     * Determina el siguiente segmento del tren basándose únicamente en la topología física de las
     * vías, siguiendo las agujas del desvío actual.
     *
     * @param head el vehículo en cabeza del tren.
     * @param graph el grafo ferroviario.
