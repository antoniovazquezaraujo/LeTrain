...
public class Gdx3DResourceContext implements Disposable {
    ...
    public void init() {
        if (modelBuilder == null) {
            ...
            tunnelPortalModel = register(modelBuilder.createBox(1.0f, 1.2f, 1.0f,
                    new Material(ColorAttribute.createDiffuse(new Color(0.15f, 0.15f, 0.15f, 1f))),
                    VertexAttributes.Usage.Position | VertexAttributes.Usage.Normal));
    ...
    private Model createOpenBox(float w, float h, float d, Color color) {
        ...
        BoxShapeBuilder.build(mpb, thickness, h, d - 2 * thickness);
