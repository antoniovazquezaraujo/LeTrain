...
public class TrackRenderer extends BaseSubRenderer {
    ...
    public TrackRenderer(Gdx3DResourceContext resourceContext, List<ModelInstance> instances,
    ...
    public void visitRailTrack(RailTrack track) {
        ...
        track.forEach(route -> {
            ...
            if (absDist >= 1 && absDist <= 3) {
                ...
                renderMultiSegmentCurve(track.getPosition(), p1, pc, p2, d1Connected && d2Connected,
                        0, resourceContext.railModel, blockedColor);
            } else {
    ...
    public void drawHalfTrack(Point pos, Dir dir, boolean connected, float shortenL, float shortenR,
        ...
        drawHalfTrackElevated(pos, dir, connected, 0.0f, shortenL, shortenR,
                resourceContext.railModel, tintColor);
    }
    ...
    public void drawHalfTrackElevated(Point pos, Dir dir, boolean connected, float elevation,
        ...
        drawHalfTrackElevated(pos, dir, connected, elevation, shortenL, shortenR, railModelToUse,
                null);
    }
    ...
    public void drawHalfTrackElevated(Point pos, Dir dir, boolean connected, float elevation,
        ...
        renderSegment(pos, new Vector3(0, 0, 0), new Vector3(dx, 0, dz), connected, elevation,
                shortenL, shortenR, railModelToUse, tintColor);
    }

    public void renderMultiSegmentCurve(Point pos, Vector3 p0, Vector3 pc, Vector3 p2,
            boolean connected, float elevation,
    ...
    public void renderMultiSegmentCurve(Point pos, Vector3 p0, Vector3 pc, Vector3 p2,
        ...
        for (int i = 0; i < numSegments; i++) {
            ...
            if (connected) {
                ...
                renderLineModel(pos, pPrev, pCurr, 0.2f + elevation,
                        resourceContext.invalidRailModel, null);
            }
        }
    }

    public void renderLineModel(Point pos, Vector3 pStart, Vector3 pEnd, float y,
            com.badlogic.gdx.graphics.g3d.Model model) {
        renderLineModel(pos, pStart, pEnd, y, model, null);
    }

    public void renderLineModel(Point pos, Vector3 pStart, Vector3 pEnd, float y,
            com.badlogic.gdx.graphics.g3d.Model model, com.badlogic.gdx.graphics.Color tintColor) {
        ...
        instance.transform.setToTranslation(pos.getX() + 0.5f + mid.x, y,
                pos.getY() + 0.5f + mid.z);
    ...
    public void renderSegment(Point pos, Vector3 pStart, Vector3 pEnd, boolean connected,
            float elevation, float shortenL, float shortenR,
            com.badlogic.gdx.graphics.g3d.Model railModelToUse) {
        renderSegment(pos, pStart, pEnd, connected, elevation, shortenL, shortenR, railModelToUse,
                null);
    }
    ...
    public void renderSegment(Point pos, Vector3 pStart, Vector3 pEnd, boolean connected,
        ...
        ballast.transform.setToTranslation(pos.getX() + 0.5f + mid.x, 0.03f + elevation,
                pos.getY() + 0.5f + mid.z);
        ...
        if (connected) {
            ...
            if (tintColor != null && !railL.materials.isEmpty()) {
                railL.materials.get(0).set(com.badlogic.gdx.graphics.g3d.attributes.ColorAttribute
                        .createDiffuse(tintColor));
            }
            railL.transform.setToTranslation(pos.getX() + 0.5f + mid.x + offX + shiftX_L,
                    0.08f + elevation, pos.getY() + 0.5f + mid.z + offZ + shiftZ_L);
            ...
            if (tintColor != null && !railR.materials.isEmpty()) {
                railR.materials.get(0).set(com.badlogic.gdx.graphics.g3d.attributes.ColorAttribute
                        .createDiffuse(tintColor));
            }
            railR.transform.setToTranslation(pos.getX() + 0.5f + mid.x - offX + shiftX_R,
                    0.08f + elevation, pos.getY() + 0.5f + mid.z - offZ + shiftZ_R);
            ...
            grader.transform.setToTranslation(pos.getX() + 0.5f + pStart.x + diff.x * 0.7f,
                    0.2f + elevation, pos.getY() + 0.5f + pStart.z + diff.z * 0.7f);
