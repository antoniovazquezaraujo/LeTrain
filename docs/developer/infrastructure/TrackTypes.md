...
Las variantes orientadas `moveSensorForward/Backward` y `moveSemaphoreForward/Backward` usan la orientación del propio elemento (`creationDir`) como sentido de avance, no el cursor: el elemento debe "descansar" siempre con su orientación alineada a un extremo real de la vía. Al avanzar, `creationDir` se rota a la dirección de salida del tramo (`continuationDir`), de modo que el elemento sigue la vía en curvas; al retroceder se usa el extremo opuesto (`backEndDir`) y la orientación apunta de vuelta al origen. De esta forma el elemento ya no queda bloqueado al llegar a una curva.
